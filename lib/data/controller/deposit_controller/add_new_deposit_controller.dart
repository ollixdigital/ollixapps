...
class AddNewDepositController extends GetxController {
  ...
  setPaymentMethod(Methods? method) {
    ...
    depositLimit =
        '${Converter.twoDecimalPlaceFixedWithoutRounding(method?.minAmount?.toString() ?? '-1')} - ${Converter.twoDecimalPlaceFixedWithoutRounding(method?.maxAmount?.toString() ?? '-1')} $currency';
  ...
  void changeInfoWidgetValue(double amount) {
    ...
    charge =
        '${MyStrings.charge}: ${Converter.twoDecimalPlaceFixedWithoutRounding('$totalCharge')} $currency';
  ...
  void submitDeposit() async {
    ...
    DepositInsertResponseModel insertModel =
        await depositRepo.insertDeposit(model);

    if (insertModel.data == null || insertModel.data?.redirectUrl == null) {
      CustomSnackBar.error(
          errorList: insertModel.message?.error ?? [MyStrings.requestFail]);
    } else {
  ...
  void showWebView(String redirectUrl) {
    Get.back();
    Get.toNamed(RouteHelper.depositWebViewScreen, arguments: redirectUrl)
        ?.then((value) {
      clearData();
      //Get.back();
    });
