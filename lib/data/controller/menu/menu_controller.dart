...
class Menu_Controller extends GetxController {
  ...
  Menu_Controller({required this.repo});
  ...
  configureMenuItem() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (model.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
        bool langStatus =
            model.data?.generalSetting?.langSwitch == '0' ? false : true;
        bool bTransferStatus =
            model.data?.generalSetting?.bTransfer == '0' ? false : true;
